#ifndef PETSCII_EXPORT_ILBM_H
#define PETSCII_EXPORT_ILBM_H

/*
 * petscii_export_ilbm.h - Export PETSCII screen as IFF ILBM image.
 *
 * Writes a standard Amiga IFF ILBM file with:
 *   - 4 bitplanes (16 colours)
 *   - CMAP palette taken from style->c64pens[].rgbcolor
 ...
 *
 * No compression (BMHD compression field = 0).
 * No datatypes or iffparse.library dependency: pure stdio + AllocVec.
 *
...
/*
 * Save the current screen as an IFF ILBM file.
 *   scr        : PETSCII screen to export
 *   style      : active style (provides the 16-colour RGB palette)
 *   path       : destination file path (should end in ".ilbm" or ".iff")
 *   withBorder : if TRUE, adds an 8-pixel C64 border strip around the image
 *
 * Image dimensions:
 *   FALSE -> width*8  x height*8   (e.g. 320x200 for a standard 40x25 screen)
